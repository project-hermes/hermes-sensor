...
ms5837::ms5837()
{
    ...
    for (uint8_t i = 0; i < 7; i++)
    {
        ...
        _log.info("%d Calibration Value %d: %d", Time.now(), i, _msCalibrationValue[i]);
    }
    ...
    {
        _log.error("%d CRC does not match!!!", Time.now());
        _log.error("Got %u but expected %u", crcCalculated, crcRead);
    }
}
...
void ms5837::readValues()
{
    ...
    _log.info("%d RawTemp: %u RawPressure: %u", Time.now(), _rawTemp, _rawPres);
}
